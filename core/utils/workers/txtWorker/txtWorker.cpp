...
int txtWorker::saveShortTxt(const requestEntity &request, const std::string &directoryPath) {
    ...
    for (const auto &p: parts) {
        if (p.name == "textTitle" || p.name == "title") title = std::string(p.data);
        if (p.isFile() || p.contentType.find("text/plain") != std::string::npos) filePart = &p;
    }

    if (!filePart) return -2;
    if (title.empty()) title = filePart->filename.empty() ? "note" : filePart->filename;
...
int txtWorker::saveLongTxt(const requestEntity &request, const std::string &directoryPath) {
    ...
    const MultipartPart *txtPart = nullptr;
    std::string title = "";

    for (const auto &p: parts) {
        if (p.name == "textTitle" || p.name == "title") title = std::string(p.data);
        if (p.isFile() || p.contentType.find("text/") != std::string::npos) txtPart = &p;
    }

    if (!txtPart) return -2;

    size_t startOffsetInFile = txtPart->data.data() - headerPreview.data();
    src.seekg(0, std::ios::end);
    size_t fileSize = src.tellg();

    size_t footerSize = boundary.length() + 8;
    long long txtSize = (long long) fileSize - startOffsetInFile - footerSize;
    if (txtSize < 0) txtSize = fileSize - startOffsetInFile;

    if (title.empty()) title = txtPart->filename.empty() ? "note" : txtPart->filename;

    std::string ext = ".txt";
    if (size_t dot = txtPart->filename.find_last_of("."); dot != std::string::npos) {
        ext = txtPart->filename.substr(dot);
    }
    ...
    size_t remaining = (size_t) txtSize;
    while (remaining > 0) {
