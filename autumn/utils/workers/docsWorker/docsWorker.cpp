...
int docsWorker::saveShortDocs(const requestEntity &request, const std::string &directoryPath) {
    ...
    for (const auto& p : parts) {
        if (p.name == "docsTitle" || p.name == "title") {
            title = std::string(p.data);
        }
        if (p.isFile() ||
            p.contentType.find("application/") != std::string::npos ||
            p.contentType.find("text/") != std::string::npos) {
            filePart = &p;
        }
    }
    ...
    if (title.empty()) {
        title = filePart->filename.empty() ? "document" : filePart->filename;
    }

    std::string ext = ".docx";
    size_t dot = filePart->filename.find_last_of(".");
    if (dot != std::string::npos) {
        ext = filePart->filename.substr(dot);
    }
    ...
    dest.write(filePart->data.data(), filePart->data.size());
    return 0;
}

int docsWorker::saveLongDocs(const requestEntity &request, const std::string &directoryPath) {
    ...
    const MultipartPart* docPart = nullptr;
    std::string title = "";

    for (const auto& p : parts) {
        if (p.name == "docsTitle" || p.name == "title") title = std::string(p.data);
        if (p.isFile() || p.contentType.find("application/") != std::string::npos) {
            docPart = &p;
        }
    }
    ...
    size_t startOffsetInFile = docPart->data.data() - headerPreview.data();

    src.seekg(0, std::ios::end);
    size_t fileSize = src.tellg();

    size_t footerSize = boundary.length() + 8;
    long long docSize = (long long)fileSize - startOffsetInFile - footerSize;
    if (docSize < 0) docSize = fileSize - startOffsetInFile;

    if (title.empty()) title = docPart->filename.empty() ? "document" : docPart->filename;

    std::string ext = ".docx";
    if (size_t dot = docPart->filename.find_last_of("."); dot != std::string::npos) {
        ext = docPart->filename.substr(dot);
    }
    ...
    size_t remaining = (size_t)docSize;
